...
bool ModePspfix::checkStockPath()
{
    ...
    if (!Fs::exists(path))
    {
        std::cout << "Please run `pandory pspfix stage1` first to install the PSP injector via USB stick on to your"
                  ...
                  << std::endl
                  << "in order to fix stock games." << std::endl;
        return false;
    }
    return true;
}
...
int ModePspfix::patchControlFolder(const std::string& source, const std::string& target, pspConfigGameDef gameDef)
{
    ...
    std::cout << "  - Copying control file: "
              << "controls" + std::to_string(gameDef.controlType) + ".ini" << std::endl;
    std::string copyTarget = target + "PSP/SYSTEM/controls.ini";
    Fs::copy(copySrc, copyTarget);

    if (editionCheck.isUltimate())
    {
        std::cout << "  - Copying setting file: "
                  << "ppsspp" + std::to_string(gameDef.ppssppSettings) + ".ini" << std::endl;
...
bool ModePspfix::stockFix()
{
    ...
    if (editionCheck.isShareware())
    {
        ...
        std::cout << "**** Did you know that ultimate-edition users also get optimized PSP performance tweaks and "
                     "extra two-player game support? ****"
                  << std::endl;
        for (int i = 10; i >= 1; i--)
        {
