...
std::string ModeAdd::getRomSuffix(const std::string& romFilename)
{
    ...
    std::string date;
    // countries
    if (std::regex_search(romFilename, std::regex("\\(.*JAPAN.*\\)", std::regex::icase)))
        countries.append("J");
    if (std::regex_search(romFilename, std::regex("\\(.*USA.*\\)", std::regex::icase)))
        countries.append("U");
    if (std::regex_search(romFilename, std::regex("\\(.*EUROPE.*\\)", std::regex::icase)))
        countries.append("E");
    if (std::regex_search(romFilename, std::regex("\\(.*BRASIL.*\\)", std::regex::icase)))
        countries.append("B");
    if (std::regex_search(romFilename, std::regex("\\(.*KOREA.*\\)", std::regex::icase)))
        countries.append("K");
    if (std::regex_search(romFilename, std::regex("\\(.*CHINA.*\\)", std::regex::icase)))
        countries.append("C");
    // flags
    if (std::regex_search(romFilename, std::regex("\\(.*DEMO.*\\)", std::regex::icase)))
        flags.append("D");
    if (std::regex_search(romFilename, std::regex("\\(.*BETA.*\\)", std::regex::icase)))
        flags.append("B");
    if (std::regex_search(romFilename, std::regex("\\(.*HACK.*\\)", std::regex::icase)))
        flags.append("H");
    if (std::regex_search(romFilename, std::regex("\\(.*PROTO.*\\)", std::regex::icase)))
        flags.append("P");
    if (std::regex_search(romFilename, std::regex("\\(.*UNL.*\\)", std::regex::icase)))
        flags.append("U");
    ...
    for (const auto& filePath : filenames)
    {
        ...
        if (result == 0)
        {
            ...
            if (!shortSystemName.empty())
            {
                ...
                targetRomName =
                    calculateRomName(i, system, filePath.string(), mcGamesXml, shortSystemName, targetRomName);
...
std::string& ModeAdd::calculateRomName(int i, const std::string& system, const std::string& filePath,
                                       McGamesXML& mcGamesXml, const std::string& shortSystemName,
                                       std::string& targetRomName)
{
