#include "modes/ModeJailbreakStick.h"
#include <iostream>
...
int ModeJailbreakStick::getJailbreakChoice()
{
    ...
    std::cout << "[2] - Pandora Games 3D+/Saga DIY (4018 Games, GB3000 board with firmware 20200704 onwards)"
              << std::endl;
    std::cout << "[3] - Pandora Key 7 (Red/Green motherboard) Requires a Micro-SD card. Will NOT work on USB stick!"
              << std::endl
              << std::endl;
    ...
    else if (jailbreakType == 50)
    { // 3dplus
        return 2;
    }
    else if (jailbreakType == 51)
    { // 7
        return 3;
    }
    return -1;
