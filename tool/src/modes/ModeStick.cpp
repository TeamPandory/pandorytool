...
int ModeStick::makeStick(const downloadDefinition& stickData)
{
    ...
    if (stickToDownload.name == "jailbreak3D" || stickToDownload.name == "jailbreak3DPlus"
        || stickToDownload.name == "jailbreak7")
    {
        int x = 0;
        ...
        if (editionCheck.isShareware() && stickToDownload.name == "jailbreak3D")
        {
            std::cout << std::endl;
            std::cout
                << "Did you know that the Pandory Jailbreak includes an additional automatic aspect-ratio switching"
                << std::endl;
            std::cout << "feature for users who have supported the project by buying us a coffee?" << std::endl;
            std::cout
                << "If you are interested in using this feature, please visit https://www.buymeacoffee.com/CKZbiXa "
                << std::endl;
