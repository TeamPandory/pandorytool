...
std::string ScreenScraper::getUrl()
{
    ...
    url = scrapeUrl + "?devid=" + devId + "&devpassword=" + devPw + "&ssid=" + username + "&sspassword=" + password
          + "&softname=SkraperUI-1.1.20154&output=xml&neoforceupdate=0"
            "&romtype=rom&romnom="
          + curlpp::escape(base) + "&md5=" + md5Hash + "&systemeid=" + std::to_string(scraperSystemId);
    //std::cout << "[scraper] getting url: " << url << std::endl;
    return url;
}
...
void ScreenScraper::removeMediaContent(tinyxml2::XMLElement* data)
{
    ...
    if (tmpGame != nullptr)
    {
        ...
        {
            for (tinyxml2::XMLElement* media = tmpMedias->FirstChildElement("media"); media != nullptr;
...
std::string ScreenScraper::downloadVideo()
{
    if (medias != nullptr)
    {
        for (tinyxml2::XMLElement* media = medias->FirstChildElement("media"); media != nullptr;
             media = media->NextSiblingElement("media"))
        {
...
{
    std::string url =
        "https://screenscraper.fr/medias/" + std::to_string(systemId) + "/" + std::to_string(gameId) + "/video.mp4";
...
std::string ScreenScraper::downloadScreenshot()
{
    std::string url = "https://screenscraper.fr/image.php?gameid=" + std::to_string(gameId)
                      + "&media=ss&hd=0&region=wor&num=&version=&maxwidth=800&maxheight=600";
    std::string targetFolder = destFolder + "/" + std::to_string(gameId);
    std::string targetFileName = targetFolder + "/" + std::to_string(gameId) + ".png";
    Fs::makeDirectory(targetFolder);
    if (!Fs::exists(targetFileName))
    {
        downloadFile(url, targetFileName);
...
void ScreenScraper::downloadFile(const std::string& url, const std::string& filename)
{
    ...
    if (curl)
    {
        ...
        curl_easy_setopt(curl, CURLOPT_REFERER, "https://screenscraper.fr/gameinfos.php?");
