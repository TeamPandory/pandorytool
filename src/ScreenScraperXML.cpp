...
std::string ScreenScraperXML::getDescription() {
    ...
    if (descriptions != nullptr) {
        for (tinyxml2::XMLElement *synopsis = descriptions->FirstChildElement("synopsis");
             synopsis != nullptr;) {
...
int ScreenScraperXML::getYear() {
    ...
    if (dates != nullptr) {
        std::string romName;
        for (tinyxml2::XMLElement *date = dates->FirstChildElement("date");
             date != nullptr;) {
...
std::string ScreenScraperXML::getGenre() {
    ...
    if (genres != nullptr) {
        for (tinyxml2::XMLElement *genre = genres->FirstChildElement("genre");
             genre != nullptr;
