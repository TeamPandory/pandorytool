...
bool ModeDXStick::replaceZip(std::string& path)
{
    return true;
}

bool ModeDXStick::downloadPatchFile(const std::string &patchName, const std::string &target)
{
    ...
    downloadFile(patchUrl + lowerCase, target + "/roms/" + patchName);
...
int ModeDXStick::startDXPatch(std::string &target) {
    ...
    if (!Fs::exists(had+".zip") || !Fs::exists(hadc+".zip") || !Fs::exists(rade+".zip")) {
        ...
        if (!dxPatches.isValidZip(had) || !dxPatches.isValidZip(hadc) || !dxPatches.isValidZip(rade)) {
            std::cout << "Your DX SD-card seems to be incompatible or has already been patched." << std::endl;
            std::cout << "Please contact @dajoho or @emuchicken via Discord" << std::endl;
            ...
            extract(hadHash.c_str());
            extract(hadcHash.c_str());
            extract(radeHash.c_str());
            Fs::remove(hadHash);
            Fs::remove(hadcHash);
            Fs::remove(radeHash);
            chdir(curDir.c_str());
